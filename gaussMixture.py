...
numpyTarget = numpy.random.uniform(-1.0, 1.0, (len(areas), N_PARTIES))
for i, area in enumerate(areas):
    for j in range(N_PARTIES):
        numpyTarget[i][j] = area._votes[j]
target = theano.shared(name='target', value=numpyTarget).astype(theano.config.floatX)
embeddings = theano.shared(name='embeddings', value=0.2 * numpy.random.uniform(0.01, 0.5, (len(region2id), N_PARTIES)).astype(theano.config.floatX))
mixture = theano.shared(name='mixture', value=0.2 * numpy.random.uniform(0.1, 0.2, (len(areas), 5)).astype(theano.config.floatX))
...
loss = theano.scan( lambda i: (target[i] - mixture[i][0]*embeddings[indices[i][0]] - mixture[i][1]*embeddings[indices[i][1]] - mixture[i][2]*embeddings[indices[i][2]] - mixture[i][3]*embeddings[indices[i][3]] - mixture[i][4]*embeddings[indices[i][4]])**2, sequences=areasIndices )[0].sum()
